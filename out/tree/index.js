...
class GistTreeProvider {
    ...
    isRootNode(node) {
        return (node instanceof nodes_1.GistsNode ||
            ...
            node instanceof nodes_1.DailyGistNode ||
    ...
    async getChildren(element) {
            ...
            if (!this.store.isSignedIn) {
                ...
                if (this.store.isLoading) {
                    ...
                    if (this.store.dailyNotes.show) {
                        nodes.unshift(new nodes_1.DailyGistNode(this.extensionContext, this.store.dailyNotes.gist));
                    }
        ...
        else if (element instanceof nodes_1.DailyGistNode) {
            if (!element.gist ||
                Object.keys(element.gist.files).filter((file) => file !== constants_1.DAILY_TEMPLATE_FILENAME).length === 0) {
                return [new nodes_1.OpenTodayNoteNode()];
            }
            else {
                const children = await getGistFiles(element.gist);
                return children.filter((child) => !(child instanceof nodes_1.GistFileNode && child.file.filename === constants_1.DAILY_TEMPLATE_FILENAME));
            }
        }
    }
    dispose() {
